...
import time
import gradio as gr
from pipeline import MiniCPM3
import argparse

parser = argparse.ArgumentParser()
parser.add_argument('-m', '--model_path', type=str,  default="llm_bmodels/minicpm3-4b_int4_seq512_1dev.bmodel", help='path to the bmodel file')
parser.add_argument('-t', '--tokenizer_path', type=str, default="llm_models/MiniCPM3/token_config", help='path to the tokenizer file')
...
def gr_update_history():
    if model.model.token_length >= model.model.SEQLEN:
        gr.Warning("reach the maximal length, MiniCPM3 would clear all history record")
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        with gr.Column():
            chatbot = gr.Chatbot(label="MiniCPM3", height=800)

            with gr.Row():
                user_input = gr.Textbox(show_label=False, placeholder="Chat with MiniCPM3", lines=1, min_width=300, scale=6)
