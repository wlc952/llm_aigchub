...
import time
import gradio as gr
from pipeline import Qwen1_5
import argparse

parser = argparse.ArgumentParser()
parser.add_argument('-m', '--model_path', type=str,  default="/data/llm/LLM-TPU/bmodels/qwen1.5-1.8b_int8_1dev.bmodel", help='path to the bmodel file')
parser.add_argument('-t', '--tokenizer_path', type=str, default="/data/llm/LLM-TPU/models/Qwen1_5/token_config", help='path to the tokenizer file')
...
def gr_update_history():
    if model.model.token_length >= model.model.SEQLEN:
        gr.Warning("reach the maximal length, Qwen1.5 would clear all history record")
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        with gr.Column():
            chatbot = gr.Chatbot(label="Qwen1.5", height=800)

            with gr.Row():
                user_input = gr.Textbox(show_label=False, placeholder="Chat with Qwen1.5", lines=1, min_width=300, scale=6)
